...
def topk_sparse_weights(logits: torch.Tensor, k: int, temperature: float = 1.0) -> torch.Tensor:
...
class WaveDensityBlock(nn.Module):
    ...
    def __init__(
        ...
        self.freqs = nn.ParameterList([
            nn.Parameter(torch.randn(self.num_masks, self.num_waves_per_mask, 2) * 4.0)
            for _ in range(self.num_heads)
        ])
        self.amps = nn.ParameterList([
            nn.Parameter(torch.randn(self.num_masks, self.num_waves_per_mask) * 0.05 + 0.8)
            for _ in range(self.num_heads)
        ])
        self.phases = nn.ParameterList([
            nn.Parameter(torch.randn(self.num_masks, self.num_waves_per_mask) * 2 * math.pi)
            for _ in range(self.num_heads)
        ])
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for h in range(self.num_heads):
            ...
            noise = torch.randn(B, self.num_samples, S, device=x.device, dtype=x.dtype) * self.noise_sigma
...
class WaveDensityAttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        seed = 1337 + (self.embed_dim * 31 + self.seq_len * 17 + self.num_heads * 13 + self.content_rank * 7)
        gen.manual_seed(seed)
        self.register_buffer("content_wq", torch.randn(self.num_heads, self.head_dim, self.content_rank, generator=gen) * scale, persistent=False)
        self.register_buffer("content_wk", torch.randn(self.num_heads, self.head_dim, self.content_rank, generator=gen) * scale, persistent=False)
        if self.learned_content:
            self.content_q_proj = nn.Linear(self.embed_dim, self.num_heads * self.content_rank, bias=False)
            self.content_k_proj = nn.Linear(self.embed_dim, self.num_heads * self.content_rank, bias=False)
        ...
        self.freqs = nn.ParameterList([
            nn.Parameter(torch.randn(self.num_masks, self.num_waves_per_mask, 2) * 1.5)
            for _ in range(self.num_heads)
        ])
        self.amps = nn.ParameterList([
            nn.Parameter(torch.randn(self.num_masks, self.num_waves_per_mask) * 0.02 + 0.5)
            for _ in range(self.num_heads)
        ])
        self.phases = nn.ParameterList([
            nn.Parameter(torch.randn(self.num_masks, self.num_waves_per_mask) * math.pi)
            for _ in range(self.num_heads)
        ])

        rel_dist_kern = torch.arange(-(self.seq_len - 1), self.seq_len).float()
        p1_k = rel_dist_kern / self.seq_len
        p2_k = torch.sign(rel_dist_kern) * torch.sqrt(torch.abs(rel_dist_kern) + 1e-6) / math.sqrt(self.seq_len)
        ...
        self.register_buffer("window_kern_pos", torch.exp(-(dpos ** 2) / (2 * (window_sigma ** 2))))

        self.use_ffn = use_ffn
        self.ffn_mult = int(ffn_mult)
        if self.use_ffn:
            hidden = self.embed_dim * self.ffn_mult
            self.ffn = nn.Sequential(nn.Linear(self.embed_dim, hidden), nn.GELU(), nn.Linear(hidden, self.embed_dim))
    ...
    def _causal_linear_attn_chunked(self, qf, kf, v, chunk_size=64):
        ...
        with torch.amp.autocast(device_type=v.device.type, enabled=False):
            qf32, kf32, v32 = qf.to(torch.float32), kf.to(torch.float32), v.to(torch.float32)
            for start in range(0, S, chunk_size):
                end = min(S, start + chunk_size)
                q, k, vc = qf32[:, :, start:end, :], kf32[:, :, start:end, :], v32[:, :, start:end, :]
                ...
                out[:, :, start:end, :] = (num / (den.unsqueeze(-1) + eps)).to(out.dtype)
    ...
    def _render_attn_kernel(self, head, weights_mod):
        ...
        with torch.amp.autocast(device_type=weights_mod.device.type, enabled=False):
            ...
            if active_mask_indices.numel() == 0:
                return torch.zeros(B, 2 * self.seq_len - 1, device=weights_mod.device, dtype=out_dtype)
            ...
            waves = (torch.sin(phase) if self.use_sin_waves else triangle_wave(phase)) * amps
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.content_mix > 0.0:
            if self.learned_content:
                q = self.content_q_proj(x).view(B, S, self.num_heads, self.content_rank).transpose(1, 2)
                k = self.content_k_proj(x).view(B, S, self.num_heads, self.content_rank).transpose(1, 2)
                content_ctx = self._causal_linear_attn_chunked(self._phi(q), self._phi(k), v)
            else:
        ...
        for h in range(self.num_heads):
            ...
            scale = 0.5 * torch.tanh(torch.einsum("br,rm->bm", coeff, self.mod_basis[h]))
            ...
            if self.use_checkpoint:
                attn_kern_batch = checkpoint(lambda wm: self._render_attn_kernel(h, wm), weights_mod, use_reentrant=False)
            else:
                attn_kern_batch = self._render_attn_kernel(h, weights_mod)
            attn_kern_pos = attn_kern_batch[:, (S - 1) : (2 * S - 1)]
            if self.use_sampling:
                noise = torch.randn(B, self.num_samples, S, device=x.device, dtype=x.dtype) * self.noise_sigma
                dens_kern_pos = self.step(attn_kern_pos.unsqueeze(1) + noise).mean(dim=1)
            else:
                dens_kern_pos = torch.sigmoid(self.attn_alpha * attn_kern_pos)
            dens_kern_pos = dens_kern_pos * self.window_kern_pos.unsqueeze(0).to(dens_kern_pos.dtype)
            kh = km[:, :, h].unsqueeze(-1)
            stacked = torch.cat([kh, kh * vh], dim=-1)
            out_stacked = self._toeplitz_causal_apply_fft_stacked(dens_kern_pos, stacked)
            head_ctx = out_stacked[:, :, 1:] / (out_stacked[:, :, 0:1] + 1e-8)
            if content_ctx is not None:
                head_ctx = (1.0 - self.content_mix) * head_ctx + self.content_mix * content_ctx[:, h]
            qh = qm[:, :, h].unsqueeze(-1)
...
class WaveCharLM(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.vocab_size, self.seq_len, self.embed_dim = int(vocab_size), int(seq_len), int(embed_dim)
        ...
        self.blocks = nn.ModuleList([
            WaveDensityAttentionBlock(
                embed_dim=self.embed_dim, seq_len=self.seq_len, num_heads=num_heads,
                num_masks=num_masks, num_waves_per_mask=num_waves_per_mask,
                topk_masks=topk_masks, attn_alpha=attn_alpha, content_rank=content_rank,
                content_mix=content_mix, learned_content=learned_content,
                use_sin_waves=use_sin_waves, ffn_mult=ffn_mult

...
\begin{itemize}
  ...
  \item Exploits Toeplitz structure for efficient causal computation (FFT-based convolution),
  \item Provides an explicit compute--locality knob via distance-windowed kernels,
  \item Separates attention pattern capacity (masks/waves) from parameter count.
\end{itemize}
...
This section summarizes the main \emph{pros} of Wave-Density Attention (WDA) as an architecture, and the primary constraints it introduces.

\paragraph{Pros.}
\begin{itemize}
  \item \textbf{Structured mixing without explicit similarity.} WDA generates attention weights from a small set of learnable wave masks and a sparse routing controller, rather than explicit query--key similarity. This provides a different inductive bias that can favor periodicity, compositional motifs, and cancellation.
  \item \textbf{Subquadratic long-context path.} When attention depends only on relative distance $d=i-j$ (Toeplitz), causal attention reduces to a 1D convolution that can be computed in $\mathcal{O}(S\log S)$ time via FFT without materializing an $S\times S$ matrix.
  \item \textbf{Explicit locality control.} Because the kernel is defined over distances, WDA can impose a soft or hard distance window directly in kernel space, providing a direct compute--quality knob that is difficult to enforce in dense dot-product attention without block-sparse infrastructure.
  \item \textbf{Mask capacity decoupled from model size.} A relatively small number of wave parameters can express diverse kernels through sparse routing and mixture composition, suggesting a route to improved \emph{capability-per-parameter} when the inductive bias matches the task.
  \item \textbf{Hardware-friendly primitive.} FFT-based convolution and dense projections map well to accelerator kernels; the mechanism avoids forming large intermediate attention matrices.
\end{itemize}

\paragraph{Trade-offs and constraints.}
\begin{itemize}
  \item \textbf{Toeplitz restriction.} Exact FFT efficiency relies on the attention pattern being a function of relative distance. This reduces expressivity compared to fully content-based pairwise attention where each query can attend differently to each key.
  \item \textbf{Content dependence is indirect.} WDA incorporates content primarily through routing and lightweight modulation; it does not compute token-to-token similarity. This can be beneficial (regularization/structure) but may limit tasks requiring fine-grained associative recall.
  \item \textbf{Inference requires caching for long contexts.} Like other causal sequence models, practical long-context decoding benefits from caching or stateful computation; otherwise decoding cost grows with context length.
\end{itemize}

...
def train_streaming_lm(
    ...
    dataset_split = str(data_cfg.get("split", "train_sft" if dataset_kind == "ultrachat" else "train"))
    dataset_val_split = str(data_cfg.get("val_split", "test_sft" if dataset_kind == "ultrachat" else "train"))
    assistant_only_loss = bool(data_cfg.get("assistant_only_loss", dataset_kind == "shared_train" if dataset_kind == "ultrachat" else False)) # Simplified
    if dataset_kind == "ultrachat":
        assistant_only_loss = data_cfg.get("assistant_only_loss", True)
    
    include_assistant_prefix_in_loss = bool(data_cfg.get("include_assistant_prefix_in_loss", True))
    dataset_name = str(data_cfg.get("dataset_name", "HuggingFaceH4/ultrachat_200k" if dataset_kind == "ultrachat" else "allenai/c4"))
    ...
    if dataset_kind == "ultrachat":
        print(f"Initializing streaming UltraChat dataset ({dataset_name}, split={dataset_split})...")
        ...
        if use_mixed_phase:
            ...
            train_stream_c4 = build_streaming_dataset(tokenizer, seq_len=seq_len, buffer_size=buffer_size)
        else:
            train_stream_c4 = None
    else:
        print("Initializing streaming C4 dataset...")
        train_stream = build_streaming_dataset(tokenizer, seq_len=seq_len, buffer_size=buffer_size)
        val_stream = build_streaming_dataset(tokenizer, seq_len=seq_len, buffer_size=buffer_size)
        train_stream_c4 = None
    ...
    if use_8bit_opt:
        if not device.startswith("cuda"):
            print("Warning: bitsandbytes 8-bit optimizer requires CUDA — falling back to AdamW.")
            ...
            try:
                ...
                print("Warning: bitsandbytes not available or failed to initialize (falling back to AdamW).")
                opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    else:
        opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    ...
    if os.path.exists(checkpoint_path):
        ...
        try:
            ...
            print(f"⚠️ Could not load optimizer state (likely due to model changes): {e}")
    ...
    for step in range(start_step, steps + 1):
        ...
        in_mixed_phase = use_mixed_phase and step >= mixed_phase_start_step and train_stream_c4 is not None
        
        if in_mixed_phase:
            x, y = sample_mixed_batch(
                train_stream, train_stream_c4,
            ...
            x, y = sample_batch(train_stream, batch_size=batch_size, seq_len=seq_len, device=device, assistant_only_loss=assistant_only_loss)
        
        opt.zero_grad(set_to_none=True)
        amp_dtype = torch.bfloat16 if use_bf16 else torch.float16
        with torch.amp.autocast(device_type=amp_device, enabled=use_amp, dtype=amp_dtype):
            logits = model(x)
        ...
        if step % val_every == 0 or step == 1:
            ...
            with torch.no_grad():
                ...
                for _ in range(val_batches):
                    vx, vy = sample_batch(val_stream, batch_size=batch_size, seq_len=seq_len, device=device, assistant_only_loss=assistant_only_loss)
                    with torch.amp.autocast(device_type=amp_device, enabled=use_amp, dtype=amp_dtype):
                        vlogits = model(vx)
                        vloss = F.cross_entropy(vlogits.view(-1, vlogits.size(-1)), vy.view(-1), ignore_index=-100)
            ...
            print(f"[{step:5d}] train {loss.item():.4f} | val {vloss:.4f} | ppl {math.exp(min(vloss, 10.0)):.2f} | {tokens_processed/1e9:.3f}B tok | {avg_step_time*1000:.0f}ms/step | ETA {eta_mins:.1f}m")
...
if __name__ == "__main__":
    ...
    MODEL_CFG = {
        "embed_dim": 768, "num_layers": 8, "num_heads": 4, "num_masks": 16, "num_waves_per_mask": 8, "topk_masks": 8, "attn_alpha": 3.0, "content_rank": 8, "content_mix": 0.15, "learned_content": True, "use_sin_waves": False, "ffn_mult": 4, "tie_embeddings": False,
    }

    DATA_CFG = {
        "dataset": "ultrachat", "dataset_name": "HuggingFaceH4/ultrachat_200k", "split": "train_sft", "val_split": "test_sft", "buffer_size": 10000, "assistant_only_loss": True, "include_assistant_prefix_in_loss": False,
    }

    TRAIN_CFG = {
        "warmup_steps": 500, "clip_grad": 0.5, "use_compile": True, "compile_mode": "reduce-overhead", "use_bf16": True, "use_mixed_phase": True, "mixed_phase_ratio": 0.1, "mixed_phase_primary_ratio": 0.8,
    }

    train_streaming_lm(device=dev, **RUN, model_cfg=MODEL_CFG, data_cfg=DATA_CFG, train_cfg=TRAIN_CFG)
